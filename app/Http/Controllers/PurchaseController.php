...
class PurchaseController extends Controller
{
    ...
    public function importPurchase(Request $request)
    {
        ...
        foreach ($product_data as $key => $product) {
            ...
            $product_purchase->net_unit_cost = number_format($net_unit_cost, 0, ',', '.');
            $product_purchase->discount = $discount[$key] * $qty[$key];
            $product_purchase->tax_rate = $tax[$key]['rate'];
            $product_purchase->tax = number_format($product_tax, 0, ',', '.');
            $product_purchase->total = number_format($total, 0, ',', '.');
            $product_purchase->save();
            $lims_purchase_data->total_qty += $qty[$key];
            $lims_purchase_data->total_discount += $discount[$key] * $qty[$key];
            $lims_purchase_data->total_tax += number_format($product_tax, 0, ',', '.');
            $lims_purchase_data->total_cost += number_format($total, 0, ',', '.');
        }
